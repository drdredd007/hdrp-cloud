    ...
    public sealed class PlanetLocalPatch : IDisposable
    {
        ...
        public static PlanetLocalPatch Build(PlanetDefinition definition,PlanetSurfaceAddress address,int2 key,double size,int resolution)
        {
            ...
            try
            {
                ...
                new PlanetLocalPatchJob {Definition=definition,Frame=frame,Key=key,Size=size,Resolution=resolution,
                    Positions=result.Positions,Normals=result.Normals,Colors=result.Colors}.Schedule(count,64).Complete();
                int index=0;
